<?php
    ...
    function form($opt,$dispname,$id,$name,$value){
    if( $opt=='T'){
    echo"  <div class=\"form-group\">\n";
    echo"    <label class=\"col-sm-3 control-label\" for=\"{$id}\">{$dispname}</label>\n";
    echo"    <div class=\"col-sm-9 form-inline\">\n";
    echo"      <input type=\"text\" class=\"form-control\" id=\"{$id}\" name=\"{$name}\" maxlength=\"80\" size=\"80\" value=\"{$value}\" placeholder=\"{$dispname}\">\n";
    echo"    </div>\n";
    echo"  </div>\n";
    }else if ( $opt=='C'){
    echo"  <div class=\"form-group\">\n";
            ...
            if( strpos($option,'A') !== false ){
                 ...
                 if( isset($settings[$key]) ){
                     foreach($settings[$key] as $value2){
                         form($opt,$dispname,$key,$key.'[]',$value2);
                         $cnt += 1;
                     }
                 }
                 for( $i = $cnt ; $i < $num ; $i++ ){
                     form($opt,$dispname,$key,$key.'[]','');
                 }
            //単純入力
            }else{
                 form($opt,$dispname,$key,$key,$settings[$key]);
            }
        }
    }
    echo"  <div class=\"form-group\">\n";
