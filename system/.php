...
function dbGetMenu($mode)
{
    ...
    if( $mode >= 0 ){
        $bland = "編集";
        $menu = array(
            "Edit posts"       => "index.php?mode=0",
            "Posts"       => "index.php?mode=2",
            "Edit fixed Page" => "index.php?mode=1",
            "Fixed pages" => "index.php?mode=3",
            "Medias"   => "index.php?mode=4\" target=\"blank",
            "Settings"           => "index.php?mode=5",
            "View site"     => "index.php",
            "Log out"     => "index.php?mode=6"
        );
    //通常時。初回は投稿画面へ
    }else{
        global $_menu;
